...
export class ArrayUtils {
   ...
   * // ]
   */
  static getUniqueByKey = <T, K extends keyof T>(
    array: T[],
    groupKey: keyof T,
    ...includeKeys: K[]
  ): Pick<T, K | typeof groupKey>[] => {
    return Object.values(
      array.reduce(
        (acc, obj) => {
          const keyValue = obj[groupKey] as string | number; // Ensure it's a valid key

          if (!acc[keyValue]) {
            acc[keyValue] = { [groupKey]: keyValue } as Pick<
              T,
              K | typeof groupKey
            >;
        ...
        {} as Record<string | number, Pick<T, K | typeof groupKey>>,
      ),
    );
  };
