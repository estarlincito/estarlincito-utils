...
export const apiFetch = async (
  ...
  const upperMethod = init.method?.toUpperCase() ?? 'GET';
  const shouldSendBody = upperMethod !== 'GET' && upperMethod !== 'DELETE';

  const ini: RequestInit = {
    ...init,
    body:
      shouldSendBody && init.body ? new URLSearchParams(init.body) : undefined,
    headers: {
      Accept: '*/*',
