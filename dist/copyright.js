 ...
 */
export const copyright = (buildYear) => {
    const date = new Date();
    const currentYear = date.getFullYear();
    if (buildYear > currentYear) {
        handleError(`buildYear (${buildYear}) cannot be greater than the current year (${currentYear}).`);
    }
    // If the build year is the same as the current year, don't show a range.
    const ownYears = buildYear === currentYear
        ? `Copyright © ${buildYear}`
